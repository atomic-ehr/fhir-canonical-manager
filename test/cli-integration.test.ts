...
describe("CLI Integration Tests", () => {
  ...
  test("fcm search with -sd shortcut", async () => {
    ...
    // Run the CLI command
    const result =
      await $`cd ${testDir} && bun ${cliPath} search -sd --limit 5`.text();
  ...
  test("fcm search with prefix matching", async () => {
    ...
    // Run prefix search
    const result =
      await $`cd ${testDir} && bun ${cliPath} search pat --limit 3`.text();
  ...
  test("fcm search with US Core shows resources from multiple packages", async () => {
    ...
    const initResult =
      await $`cd ${testDirUsCore} && bun ${cliPath} init`.text();
    console.log("Init result:", initResult);

    // Search for Patient resources - should find from multiple packages
    const result =
      await $`cd ${testDirUsCore} && bun ${cliPath} search patient -sd --limit 10`.text();
    ...
    const resourceLines = lines.filter(
      (line) => line.includes("http://") && line.includes('"package":'),
    );
